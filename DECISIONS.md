...
## 2026-04-16 - Keep the first Generic Host integration small: configuration-bound hosted service plus fail-fast startup
- Context: `ConnectionManager.DisposeAsync()` and `DeviceBootstrapper.StartAsync()` / `StartWithReportAsync()` already existed, but `AddCommLibCore()` still only registered services and did not participate in Generic Host lifecycle. The immediate goal was to let host-based apps bootstrap and dispose the runtime without custom glue while keeping the branch narrow.
- Decision: add configuration-bound `AddCommLibCore(IConfiguration, ...)` overloads that bind `CommLibOptions` from either the root configuration or the `CommLib` section, then register a new `CommLibHostedService` only on that path. In the first version, `CommLibHostedService.StartAsync()` filters to enabled raw profiles and reuses fail-fast `DeviceBootstrapper.StartAsync()`, while `StopAsync()` reuses `ConnectionManager.DisposeAsync()` through `IAsyncDisposable`.
- Why: this is the smallest structurally correct Generic Host bridge because it keeps bootstrap/disposal policy in the existing application/infrastructure seams, avoids widening the always-on `AddCommLibCore()` registration path, and matches normal host-start semantics where a required runtime dependency should fail startup rather than silently degrade.
- Consequences: configuration-bound host registrations now start and stop CommLib automatically, disabled raw profiles are ignored before mapping/validation, and a future report-returning/partial-startup policy can be added later only if a concrete deployment need appears.
