...
## 2026-04-13 - Keep `ReconnectOptions` naming for now and clarify the contract instead of renaming
- Context: after runtime receive failure was made terminal and queue/hosting hardening progressed, the remaining truthfulness question was whether `ReconnectOptions` / `DeviceProfile.Reconnect` should be renamed because the live implementation only retries transport open during `ConnectAsync()`. The names already appear in config-bound types and samples, so even a compatibility alias would widen the public surface.
- Decision: do not add an alias or rename yet. Keep the current names for compatibility, and clarify through XML docs and sample-facing docs that the contract is connect-time transport-open retry only, not live-session auto-recovery.
- Why: this is the smallest truthful change that removes ambiguity without paying compatibility churn before there is evidence that external consumers or a stable package process actually need a stronger rename path.
- Consequences: current callers keep the same config/property names, documentation now points to the correct semantics, and any future alias or breaking rename stays an explicit later decision driven by real API pressure rather than by cleanup instinct.
