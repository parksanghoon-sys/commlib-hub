...
## 2026-04-18 - Keep `ReconnectOptions` as the public name for now, but strengthen doc-only clarification that it is connect-time-only
- Context: after the inbound-frame cleanup, the next truthfulness question was whether the public `ReconnectOptions` / `DeviceProfile.Reconnect` naming had become misleading enough to require a staged alias/deprecation path. Inventory across code, docs, samples, and tests showed that the runtime already consistently treated the setting as connect-time transport-open retry only, and the repo already had partial wording to that effect.
- Decision: do not add a new alias, obsolete shim, or breaking rename in this slice. Strengthen the public documentation instead so the repo now states explicitly that `Reconnect` applies only while `ConnectAsync()` is still opening the transport and does not imply automatic recovery after a later receive-loop failure.
- Why: this is the smallest truthful change that aligns the published contract with the implementation we actually ship, without creating API churn before there is stronger evidence of external compatibility pressure.
- Consequences: the runtime contract is now clearer to readers and example users, the existing tests remain the behavioral source of truth for connect-time retry vs. terminal post-connect failure, and any future rename can be treated as a separate compatibility decision instead of being mixed into documentation cleanup.

## 2026-04-18 - Parallelize only `DeviceBootstrapper.StartAsync()` and keep reporting semantics on `StartWithReportAsync()`
- Context: after the reconnect-contract clarification landed, the next evidence-ready runtime gap was multi-device startup latency. `DeviceBootstrapper.StartAsync()` still awaited each `ConnectAsync()` serially, but the repo already had `StartWithReportAsync()` as the explicit continue-and-report path for partial failures.
...
## 2026-05-19 - Add a generic `BinaryFrame` envelope without moving payload bit semantics into `IProtocol`
- Context: the user asked whether CommLib should manage bytes at the bit level to make protocols extensible like Modbus, then clarified that the goal is a generic structure rather than a dedicated Modbus implementation. The repo already had `BitFieldPayloadSchema` for payload-level bit/byte interpretation, while `IProtocol` only handled frame boundaries through `LengthPrefixedProtocol`.
- Decision: add the first generic frame envelope as `ProtocolTypes.BinaryFrame`, with configuration for optional start bytes, 1/2/4-byte payload length prefixes, and optional CRC16/Modbus checksum coverage. Keep address/function/register/bitfield interpretation in the payload layer through `RawHex` and `BitFieldPayloadSchema` instead of expanding `IProtocol` into a device-command DSL. Preserve a pre-registered `IProtocolFactory` in `AddCommLibCore()` so C# implementations remain the escape hatch when configuration is not enough.
- Why: this gives real binary-device framing flexibility now while preserving the clean boundary between transport frame parsing and payload semantics. It also leaves a C# escape hatch through additional `IProtocol` implementations without forcing every future protocol variation into a premature JSON DSL.
- Consequences: CommLib now supports more than `LengthPrefixed` framing, but it still is not a full Modbus stack. Future delimiter, escaping, fixed-length, alternate checksum, or message-header features should be added only from concrete device contracts.

## 2026-05-19 - Start the span/minimal-copy pipeline as additive fast paths, not breaking contract replacement
- Context: after the first generic `BinaryFrame` slice, the user asked to manage data more broadly through `Span`-based value processing with minimal copies. The current pipeline already has some memory-friendly boundaries (`ITransport.SendAsync(ReadOnlyMemory<byte>)`, binary payload models, and bitfield codecs), but `IProtocol.TryDecode`, `ISerializer.Serialize`, `MessageFrameDecoder`, `MessageFrameEncoder`, and `TransportMessageReceiver` still force intermediate arrays or repeated pending-buffer copies.
- Decision: plan the next slice around additive optional contracts: zero-copy protocol decode using `ReadOnlyMemory<byte>` payload slices, receiver pending-buffer windowing, and outbound span writer paths for built-in serializers/protocols. Keep the current `IProtocol` and `ISerializer` methods as compatibility fallbacks for custom implementations.
- Why: this reduces the hottest avoidable copies without breaking existing extension points or forcing a larger ownership/lifetime API such as pooled memory into the first span pass.
- Consequences: the implementation plan can improve built-in high-throughput paths while custom protocols and serializers continue to work unchanged; any future move to `ArrayPool<byte>`, `IMemoryOwner<byte>`, or `IBufferWriter<byte>` remains a separate ownership-design decision after this lower-risk fast-path slice is validated.

## 2026-05-20 - Keep byte/start-bit/end-bit extraction as a thin payload helper
- Context: the user clarified that the generic binary frame work still needs convenient bit-level extraction by byte position and start/end bit, but asked to avoid making the design too complex and to remove or consolidate duplicated logic where possible.
- Decision: keep `BinaryFrameProtocol` focused on the byte envelope and add only thin helpers to the existing payload bitfield layer: `BitFieldDefinition.FromByteBits(...)` centralizes byte-local bit range conversion, and `BitFieldCodec.ReadUnsigned<T>` / `ReadSigned<T>` convert existing scalar reads into requested integral target types.
- Why: this satisfies the ergonomic need without introducing a second protocol DSL, duplicate bit-walking code, or a parallel schema model. Existing `BitFieldPayloadSchema` remains the config-backed path, and code-first users now have a direct byte/start/end API.
- Consequences: generic extraction is available for code paths, but schema config still uses `BitOffset` / `BitLength` until a concrete config consumer proves that byte/start/end fields are worth the extra schema complexity.

## 2026-05-20 - Return a public session facade and remove the runtime outbound queue
- Context: `DESIGN_REVIEW.md` correctly identified that `IConnectionManager.GetSession()` exposed `IDeviceSession`, while `IDeviceSession` also exposed runtime lifecycle methods (`TryCompleteResponse`, `FailPendingResponses`, `TryDequeueOutbound`). The same design forced `ConnectionManager.SendAsync(...)` to enqueue through `DeviceSession.Send(...)` and immediately dequeue again, which allowed public session sends to leave stale outbound messages for the next manager send.
- Decision: make `IDeviceSession` a public send-oriented contract only, return a `ConnectionManager`-owned facade from `GetSession()`, remove the outbound channel from `DeviceSession`, and have `ConnectionManager` send directly through `TransportMessageSender`. Keep `DeviceSession` as the runtime pending request/response tracker used by `ConnectionManager`.
- Why: this is the smallest structurally correct fix for the real API/correctness risk. It prevents external callers from manipulating pending lifecycle state, removes the misleading one-message queue round trip, and keeps request/response tracking close to the receive pump without introducing a new factory abstraction.
- Consequences: public callers still use `GetSession().Send(...)` and `GetSession().Send<TRequest,TResponse>(...)`, but those calls now perform real transport sends through the manager. Runtime lifecycle methods remain available only on the application-level `DeviceSession` object held internally by `ConnectionManager`. The lower-priority `ProtocolOptions` and diagnostics package-boundary review items stay deferred until concrete external pressure appears.

## 2026-05-20 - Implement span/minimal-copy as optional fast paths with caller compatibility shims
- Context: the span/minimal-copy plan targeted avoidable payload and frame copies in `IProtocol.TryDecode`, `MessageFrameDecoder`, `TransportMessageReceiver`, `MessageFrameEncoder`, and the built-in serializers. The existing public `IProtocol` and `ISerializer` contracts are already extension points for custom protocol/serializer implementations, so replacing them directly would create unnecessary compatibility risk.
- Decision: add optional contracts beside the existing interfaces: `IZeroCopyProtocol` exposes decoded payload memory slices, `IFrameEncodingProtocol` describes the final frame layout and prefix/suffix write order, and `ISpanSerializer` writes payload bytes into caller-provided spans. Built-in protocols and serializers implement those contracts, while legacy `Encode`, `TryDecode(ReadOnlySpan<byte>, ...)`, and `Serialize` remain the fallback paths. `MessageFrameDecoder` also keeps an explicit `byte[]` overload so existing array call sites do not become ambiguous after adding the memory overload.
- Why: this captures the highest-value copy reductions without forcing a pooled-memory lifetime model or breaking custom implementations. The array shim is necessary because `byte[]` can convert to both `ReadOnlySpan<byte>` and `ReadOnlyMemory<byte>` when overloads have the same out parameters.
- Consequences: built-in `LengthPrefixed`/`BinaryFrame` plus `RawHex`/`NoOp` now use lower-copy receive and send paths where possible, but CommLib still does not claim full zero-copy ownership across serializer deserialization or application message models. Any future move to `ArrayPool<byte>`, `IMemoryOwner<byte>`, or `IBufferWriter<byte>` remains a separate ownership/lifetime decision.

## 2026-05-20 - Treat span review findings as targeted guardrails, not a new pipeline redesign
- Context: `SPAN_PIPELINE_REVIEW.md` identified mostly non-blocking follow-ups after the span/minimal-copy slice: partial fast-path tests, dead null-coalescing cleanup, clearer writer-order documentation, CRC performance, and fast-path observability.
- Decision: implement only the low-risk guardrails now: partial fallback tests, `ProtocolFactory` cleanup, and clearer XML comments for frame writer ordering plus byte-local bit extraction. Keep CRC table optimization and runtime fast-path observability deferred until profiling or production diagnostics requirements justify them.
- Why: the reviewed implementation is already functionally correct, and broadening into performance tuning or diagnostics counters would widen the slice without evidence. The tests and contract comments reduce realistic regression and misuse risk at low cost.
- Consequences: the span path is better specified and easier to maintain, while future CRC and observability work remain separate decisions tied to measured need or the existing production diagnostics backlog.
